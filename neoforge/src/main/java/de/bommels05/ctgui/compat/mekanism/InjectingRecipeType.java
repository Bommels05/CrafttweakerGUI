...
public class InjectingRecipeType extends SupportedRecipeType<BasicInjectingRecipe> {
    ...
    public InjectingRecipeType() {
        ...
        addAreaScrollAmountEmptyRightClick(88, 19, 17, 17, (r, am) -> {
            return new BasicInjectingRecipe(r.getItemInput(), r.getChemicalInput(), convertToUnset(am.asStack()), r.perTickUsage());
        }, r -> {
    ...
    public String getCraftTweakerString(BasicInjectingRecipe recipe, String id) {
        return "<recipetype:mekanism:injecting>.addRecipe(\"" + id + "\", " + getCTString(MekanismRecipeUtils.of(recipe.getItemInput())) + ", " + MekanismRecipeUtils.getCTString(recipe.getChemicalInput()) + ", " + getCTString(recipe.getOutputRaw()) + ", " + recipe.perTickUsage() + ");";
    }
}
