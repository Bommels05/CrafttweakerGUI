...
import de.bommels05.ctgui.mixin.EmiBrewingRecipeAccessor;
...
public class BrewingRecipeType extends SupportedRecipeType<BrewingRecipe> {
    ...
    public BrewingRecipeType() {
        ...
        addAreaEmptyRightClick(39, 36, 17, 17, (r, am) -> {
            return new BrewingRecipe(r.getId(), am.ensureAmount(1, 1).ingredient(), r.getReagent(), r.getOutput());
        }, r -> {
            return new AmountedIngredient(r.getInput(), 1);
        });
        addAreaEmptyRightClick(62, 2, 17, 17, (r, am) -> {
            return new BrewingRecipe(r.getId(), r.getInput(), am.ensureAmount(1, 1).ingredient(), r.getOutput());
        }, r -> {
            return new AmountedIngredient(r.getReagent(), 1);
        });
        addAreaScrollAmountEmptyRightClick(85, 36, 17, 17, (r, am) -> {
            return new BrewingRecipe(r.getId(), r.getInput(), r.getReagent(), am.asStack());
        }, r -> {
    ...
    public BrewingRecipe onInitialize(@Nullable BrewingRecipe recipe) throws UnsupportedRecipeException {
        ...
        if (recipe == null) {
            return new BrewingRecipe(nullRl(), Ingredient.EMPTY, Ingredient.EMPTY, ItemStack.EMPTY);
        }
        return null;
    }
    ...
    @Override
    public JsonObject getRecipeJson(BrewingRecipe recipe) {
        JsonObject json = new JsonObject();
        json.add("input", recipe.getInput().toJson());
        json.add("reagent", recipe.getReagent().toJson());
        json.add("output", getJson(recipe.getOutput()));
        return null;
    }

    @Override
    public BrewingRecipe getWithId(BrewingRecipe r, ResourceLocation id) {
        return new BrewingRecipe(id, r.getInput(), r.getReagent(), r.getOutput());
    }

    @Override
    public Function<EmiRecipe, BrewingRecipe> getAlternativeEmiRecipeGetter() {
        return recipe -> recipe instanceof EmiBrewingRecipeAccessor r ? new BrewingRecipe(nullRl(), EmiViewerUtils.getElseEmpty(r.getInput()), EmiViewerUtils.getElseEmpty(r.getIngredient()), r.getOutput().getItemStack()) : null;
    }
