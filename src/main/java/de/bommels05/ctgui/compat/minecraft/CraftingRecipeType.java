...
public class CraftingRecipeType extends SupportedRecipeType<CraftingRecipe> {
    ...
    public CraftingRecipeType() {
        ...
        addOption(shapeless, (r, value) -> {
            if (value) {
                if (r instanceof ShapedRecipe recipe) {
                    return new ShapelessRecipe(recipe.getGroup(), recipe.category(), recipe.getResultItem(regAccess()), NonNullList.copyOf(recipe.getIngredients().stream().filter(ingredient -> !ingredient.isEmpty()).toList()));
                }
            } else {
                if (r instanceof ShapelessRecipe recipe) {
    ...
    private ShapelessRecipe setIngredient(ShapelessRecipe recipe, int index, Ingredient ingredient) {
        ...
        return new ShapelessRecipe(recipe.getGroup(), recipe.category(), recipe.getResultItem(regAccess()), NonNullList.copyOf(ingredients));
    }
