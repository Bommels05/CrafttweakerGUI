...
public class TagRecipe implements Recipe<Container> {
    ...
    public TagRecipe(TagKey<?> tag) {
        ...
        Map<ResourceLocation, List<TagLoader.EntryWithSource>> tags = new TagLoader<>(null, TagManager.getTagDir(tag.registry()))
                .load(ServerLifecycleHooks.getCurrentServer().getResourceManager());
        ...
        if (entries != null) {
            for (TagEntry entry : entries.stream().map(TagLoader.EntryWithSource::entry).toList()) {
                if (entry.isTag()) {
                    if (item) {
                        itemTags.add(TagKey.create(Registries.ITEM, entry.getId()));
                    } else {
                        fluidTags.add(TagKey.create(Registries.FLUID, entry.getId()));
                    }
                } else {
                    Optional<?> optional = BuiltInRegistries.REGISTRY.get(tag.registry().location()).getOptional(entry.getId());
